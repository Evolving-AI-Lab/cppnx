...
namespace NEAT
{
	...
	template<class Type>
	void FastNetwork<Type>::setActivationFunction(const string &nodeName, ActivationFunction function)
	{
		activationFunctions[nodeNameToIndex[nodeName]] = function;
	}
    ...
    template<class Type>
    void FastNetwork<Type>::update(int iterations)
    {
    ...
    template<class Type>
    Type FastNetwork<Type>::runActivationFunction(Type tmpVal,ActivationFunction function)
    {
        Type retVal;
...
#endif
               // double x2 = tmpVal * tmpVal; double x4 = x2 * x2;
               // retVal = (1.995939420035614 + 0.3518336357159904 * x2 +
               //   0.3698673724861037 * x4) / (
               //   1.0 + 0.6413839518669078 * x2 +
               //   0.3490587153914497 * x4);

                //tmpVal = tmpVal*2.5;
                //retVal = exp(-((tmpVal*tmpVal)/1));
                retVal = 2*exp(-(pow(tmpVal,2)))-1;
        ...
        case ACTIVATION_FUNCTION_LINEAR:
            if (signedActivation)
            {
                ...
                retVal = min(max(tmpVal,(Type)-1.0),(Type)1.0);
                //retVal = tmpVal;
            }
            else
            {
