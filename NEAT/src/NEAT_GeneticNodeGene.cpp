...
namespace NEAT
{
	...
	activationFunction(_activationFunction)
	{
		if (randomizeActivation)
		{
			...
			{
				if (Globals::getSingleton()->getParameterValue("OnlySigmoidHiddenNodes")>0.5)
				{
					cerr << "You can't ask request hidden nodes to be only guassian and also ask for them to be only sigmoid." << endl;
					exit(9);
				}
				
 				activationFunction = ACTIVATION_FUNCTION_GAUSSIAN;
			}
			else if (Globals::getSingleton()->getParameterValue("OnlySigmoidHiddenNodes")>0.5)
			{
			...
			else
			{
				...
				while (
							 activationFunction==ACTIVATION_FUNCTION_ABS_ROOT||				//don't allow anything here that is not commented out
							 ...
							 //activationFunction==ACTIVATION_FUNCTION_LINEAR||				//gaussian is not on this list, so it is automatically ok
