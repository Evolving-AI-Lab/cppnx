...
namespace NEAT
{
    ...
    {
        //*NOTE* The best individual actually gets copied into the species so that the original can be deleted.
        shared_ptr<GeneticIndividual> bestIndividualEver;

        int ID;

        //Individuals currently in the species.
        //*NOTE* These objects aren't duplicated, so you cannot depend on them once the
        //generation is rolled over!
    ...
    public:
        ...
        {
            return currentIndividuals.size();
        }
