...
public:
    ...
    dbg_ostream(const dbg_ostream &rhs): std::ostream(&dbg_buf), streams(rhs.streams), dbg_buf(streams) {}
#else
...
public:
    ...
    //Added by Joost
    /**
     * Returns true if writing to out() will write to at least one stream
     */
    bool hasAttachedStream(dbg::level lvl);
...
/******************************************************************************
 * Added by Joost Huizinga
 *****************************************************************************/
bool dbg::enabled(level lvl, dbg_source src){
    return source_map[src].hasAttachedStream(lvl);
}
...
//Added by joost
bool source_info::hasAttachedStream(dbg::level lvl){
    if (lvl == dbg::none || !enabled(lvl))
    {
        return false;
    }
